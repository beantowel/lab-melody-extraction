...
from modelFeng.model import MultiDNN, MultiTaskLoss
...
def init():
    ...
    trainSeg_set = Segments_Dataset(
        train_set, SR, FRAME_SIZE, HOP_SIZE, transform=transform)
    validSeg_set = Segments_Dataset(
        valid_set, SR, FRAME_SIZE, HOP_SIZE, transform=transform)
    ...
    # prepare model
    model = MultiDNN(DIM_FEATURE, DIM_HIDDEN,
                     NUM_PITCH, NUM_VOICE, DROPOUT).to(DEVICE)
    # create log file
    print(
...
def train_epoch(model, train_loader, optimizer, lossFunction):
    ...
    for batch in tqdm(train_loader, mininterval=2, desc='  - (Training)', leave=False):
        # get data
        feature = batch['features'].squeeze(1).to(DEVICE)
